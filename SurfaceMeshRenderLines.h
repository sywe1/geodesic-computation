...
namespace OpenGP {
	...
	class Segmentslines : public SceneObject {
	public:
	...
	public:
		Segmentslines() { }
		Segmentslines(const Segments& segments) { load(segments); }
		Segmentslines(const MatMxN& P1, const MatMxN& P2) { load(P1, P2); }
		Segmentslines(const std::vector<Vec3>& segments) { load(segments); }
	public:
		HEADERONLY_INLINE void load(const MatMxN& P1, const MatMxN& P2);
		HEADERONLY_INLINE void load(const Segments& segments);
		HEADERONLY_INLINE void load(const std::vector<Vec3>& segments);
		HEADERONLY_INLINE void set_color(const Vec3& v) { this->color = v; }
		/// @} 
	...
	public:
		...
		HEADERONLY_INLINE void update(Segments& segments);
	...
	void Segmentslines::load(const MatMxN &P1, const MatMxN &P2) {
	...
	void Segmentslines::load(const Segmentslines::Segments &segments) {
	...
	void Segmentslines::load(const std::vector<Vec3>& segments)
	{
		CHECK(segments.size() % 2 == 0);
		_data.resize(3, segments.size());
		for (uint i = 0; i < segments.size(); i++)
		{
			_data.col(i) = segments[i];
		}
	}

	void Segmentslines::init() {
		///--- Shader
	...
	void Segmentslines::update(Segments& segments) {
		Segmentslines::load(segments);
		_buffer_vpos.upload_raw(_data.data(), _data.cols());
	}


	void Segmentslines::display() {
	...
	Box3 Segmentslines::bounding_box() {
		Box3 box;
